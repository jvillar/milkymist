 ...
 */
typedef struct {
	unsigned char		et_dest[6];	/* Destination node		*/ // WARNING: direct access though pointer to the packed struct will always be unaligned.
	unsigned char		et_src[6];	/* Source node			*/ // WARNING: direct access though pointer to the packed struct will always be unaligned.
...
typedef struct {
	...
	IPaddr_t		ip_src;		/* Source IP address		*/ // WARNING: direct access though pointer to the packed struct will always be unaligned.
	IPaddr_t		ip_dst;		/* Destination IP address	*/ // WARNING: direct access though pointer to the packed struct will always be unaligned.
...
#   define RARPOP_REPLY	    4			/* Response to previous request */
	 ...
	 */
	unsigned char		ar_data[0];	// WARNING: direct access though pointer to the packed struct will always be unaligned.
} ARP_t;
...
typedef struct icmphdr {
	...
	union {
		...
		unsigned long	gateway; // WARNING: direct access though pointer to the packed struct will always be unaligned.
		struct {
...
/* Copy ip address */
extern void	NetCopyIP(volatile unsigned char *, unsigned char *);
...
/* Checksum */
extern int	NetCksumOk(unsigned short *, int);	/* Return true if cksum OK	*/
extern unsigned	short NetCksum(unsigned char *, int);		/* Calculate the checksum	*/
