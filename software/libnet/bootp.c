...
BootpTimeout(void)
{
	if (BootpTry >= TIMEOUT_COUNT) {
		...
		NetSetTimeout (TIMEOUT * 1000, BootpTimeout);
		BootpRequest ();
	}
}
...
BootpRequest (void)
{
	 ...
	 *	plus the current time in msecs.
	 */
	...
	BootpID += now.sec*1000000+now.usec; //FIXED
	bp->bp_id = BootpID;
	...
	NetSetTimeout(SELECT_TIMEOUT * 1000, BootpTimeout);
